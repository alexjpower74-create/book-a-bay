...
function seed() {
  ...
  return {
    ...
    requests: [
      make('demo-requested-sample', 'requested', 'oil', '2026-09-15', '09:30', [1], {
        customer: customer('Pat Sample (sample)', '709-555-0142', '2016', 'Toyota', 'Corolla', 'Oil light came on yesterday.'),
      }),
      make('demo-confirmed-sample', 'confirmed', 'brakes', '2026-09-16', '10:00', [1], {
        customer: customer('Jordan Byrne (sample)', '709-555-0117', '2019', 'Honda', 'Civic', 'Squeal from the front left wheel.'),
      }),
...
function holds(store, excludeId) {
  ...
  for (const r of store.requests) {
    ...
    if (r.status === 'requested' || r.status === 'confirmed')
      out.push({ date: r.date, start: r.start, end: r.end, bays: r.bays, isRequest: true })
...
function view(r) {
  return {
    ...
    label: `${dayLabel(r.date)}, ${clockLabel(r.start)}`,
    offer:
      r.status === 'offered' && r.offer
        ? {
            date: r.offer.date,
            time: hhmm(r.offer.start),
            end: hhmm(r.offer.end),
            label: `${dayLabel(r.offer.date)}, ${clockLabel(r.offer.start)}`,
          }
        : null,
    shop_note: r.shop_note,
    customer: r.customer,
    shop: { name: SHOP_NAME, sample: true },
    ics_url: r.status === 'confirmed' ? `/api/r/${r.token}/ics` : null,
...
export async function handle(method, path, body = {}) {
  ...
  if (method === 'GET' && p === '/api/days') {
    ...
    const days = Array.from({ length: SETTINGS.window_days }, (_, i) => {
      ...
      return {
        ...
        open: info.open,
        reason: info.reason,
        available: info.open ? slotsFor(store, sv, date).length : 0,
      }
    })
  ...
  if (method === 'GET' && p === '/api/slots') {
    ...
    return res(200, {
      ...
      open: info.open,
      reason: info.reason,
      slots: slotsFor(store, sv, date).map(({ time, label }) => ({ time, label })),
    })
  }
  ...
  if (method === 'POST' && p === '/api/requests') {
    ...
    if (start === null || !candidateStarts(sv, body.date).includes(start))
      return bad('time', 'That time is not open for booking. Please pick another.')
    ...
    const r = {
      ...
      bays: freeBays(holds(store), body.date, start, sv.minutes, sv.bays_needed),
      offer: null,
      shop_note: '',
      customer: Object.fromEntries(['name', 'phone', 'year', 'make', 'model', 'note'].map((k) => [k, String(body[k] || '').trim()])),
      updated_at: NOW,
    }
  ...
  if (method === 'GET' && action === 'days') {
    const days = Array.from({ length: SETTINGS.window_days }, (_, i) => {
      ...
      return {
        ...
        open: info.open,
        reason: info.reason,
        available: info.open ? slotsFor(store, r.service, date, r.id).length : 0,
      }
    })
  ...
  if (method === 'GET' && action === 'slots') {
    ...
    return res(200, {
      ...
      open: info.open,
      reason: info.reason,
      slots: slotsFor(store, r.service, date, r.id).map(({ time, label }) => ({ time, label })),
    })
  }
  ...
  if (method === 'POST' && action === 'accept') {
    ...
    if (!['requested', 'offered', 'confirmed'].includes(r.status))
      return badState(`This request is ${r.status}, so it cannot be cancelled.`)
    r.status = 'cancelled'
    r.offer = null
  } else {
    return NOT_FOUND
