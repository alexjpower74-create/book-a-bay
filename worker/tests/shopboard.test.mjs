...
test('toPatch maps a request row field for field, cut to Shop Board limits', () => {
  ...
  const noted = toPatch(
    row({ note: 'Rattle at the back.', bays: [1, 2], service_name: 'Truck or RV service', minutes: 120, start_min: 765 }),
    AT,
  )
...
test('CSV export has the Shop Board columns and quotes what needs quoting', () => {
  ...
  assert.equal(
    lines[1],
    'bab-0123456789abcdef,2026-09-15,9:00,2,Pat Sample (sample),709-555-0142,2016,Toyota,Corolla,"Oil change (booked online for 9:30 AM) — Said ""soon"", please",30m',
  )
...
test('contract: the fake refuses what Shop Board refuses (validatePatch at 25c6faa)', async () => {
  ...
  const good = () => ({
    fields: { date: '2026-09-15', slot: '9:00', bay: '1', name: 'Pat Sample (sample)', estTime: '30m' },
    at: AT,
    by: 'Book a Bay',
  })
  const refusals = [
    [
      ...
      'slot must be one of 8:00, 9:00, 10:00, 11:00, 12:00 PM, 12:30 PM, 1:00, 2:00, 3:00, 4:00, 5:00',
    ],
    [
      'bab-okay-02',
      (b) => {
    ...
    [
      'bab-okay-03',
      (b) => {
        b.fields.estTime = '45m'
      },
      'estTime must be one of 30m, 1h, 2h, 3h, 4h, Day or empty',
    ],
    [
      'bab-okay-04',
      (b) => {
    ...
    [
      ...
      'A new booking must include date and slot',
    ],
    ['ab', () => {}, 'Bad booking id'],
  ]
...
test('contract: mapped patches land, an occupied row is 409 taken, a covering job is 409 busy', async () => {
  ...
  for (const [i, time] of starts.entries()) {
    const item = shopBoardItem(
      row({ id: `r_map${i}000000`, date: `2026-10-0${i + 1}`, start_min: parseTime(time), minutes: [30, 45, 60, 120, 200, 300][i] }),
      AT,
    )
  ...
  assert.deepEqual(await put(base, second.id, second.patch), {
    status: 409,
    body: { error: '9:00 is taken', code: 'taken', conflict: { slot: '9:00', bay: '2' } },
  })

  const long = {
    fields: { date: '2026-09-15', slot: '12:30 PM', bay: '2', name: 'Long job (sample)', estTime: '2h' },
    at: AT,
    by: 'Front desk',
  }
  ...
  assert.deepEqual(await put(base, covered.id, covered.patch), {
    status: 409,
    body: {
      error: 'Bay 2 is busy until 3:00 with Long job (sample)',
      code: 'busy',
      conflict: { slot: '12:30 PM', bay: '2', until: '3:00', with: 'Long job (sample)' },
    },
  })
})
